...
export class QqConversationNode extends ConversationBridge {
  ...
  async sendText(text) {
    ...
    if (isPromptMessage) {
      ...
      console.log('[QQ BUTTON] send prompt msg with keyboard, scope=%s peer=%s len=%d', scope, peerId, String(text).length)
      try {
        const result = await this._sendMarkdown(peerId, text, { scope, msgId: replyMsgId, keyboard })
        console.log('[QQ BUTTON] result: %o', result)
        return result
      } catch (err) {
        console.log('[QQ BUTTON] error: %s', err?.message ?? err)
        return this._sendMarkdown(peerId, text, { scope, keyboard })
      }
    }
