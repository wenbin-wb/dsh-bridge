...
export class QqConversationNode extends ConversationBridge {
  ...
  async _handleInbound(event) {
    ...
    // 快速预检查：白名单非空时，未授权发件人/群直接忽略
    if (!this.isAllowed(authId) && this.config.allowFrom.length > 0) {
      this.logger?.info?.(`[dsh-bridge qq] ignore ${isGroup ? 'group' : 'message'} from non-allowlisted ${isGroup ? 'group' : 'sender'} ${authId}`)
      return
    }
  ...
  async _handleInteraction(event) {
    ...
    // 快速预检查：白名单非空时，未授权发件人/群直接忽略
    if (!this.isAllowed(authId) && this.config.allowFrom.length > 0) {
      this.logger?.info?.(`[dsh-bridge qq] ignore interaction from non-allowlisted ${isGroup ? 'group' : 'sender'} ${authId}`)
      return
    }
