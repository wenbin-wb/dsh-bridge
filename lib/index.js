...
class BridgeService {
  ...
  async getStatus() {
    ...
    const tokenSuffix = (this.authManager?.enabled && this.authManager?.mode !== 'password_only' && this.authManager?.secretToken)
      ? `?auth=${this.authManager.secretToken}`
      : '';

    const baseLanUrl = lanIp ? `http://${lanIp}:${this.proxyPort}` : null;
    const lanUrl = baseLanUrl ? `${baseLanUrl}${tokenSuffix}` : null;

    const baseCloudflaredUrl = this.cloudflared?.url || null;
    const cloudflaredUrl = baseCloudflaredUrl ? `${baseCloudflaredUrl}${tokenSuffix}` : null;

    const baseCustomUrl = this.customTunnel?.publicUrl || null;
    const customUrl = baseCustomUrl ? `${baseCustomUrl}${tokenSuffix}` : null;
