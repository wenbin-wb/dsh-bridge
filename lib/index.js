...
import { installAbortSignalCompat } from './compat.js';
...
<script data-dsh-bridge-polyfill="2">!function(){try{if(typeof self.AbortSignal==="function"){var S=self.AbortSignal;if(typeof S.any!=="function"){S.any=function(s){var l=Array.prototype.slice.call(s||[]);var c=new self.AbortController();for(var i=0;i<l.length;i++){if(l[i]&&l[i].aborted){try{c.abort(l[i].reason)}catch(e){c.abort()}return c.signal}}var on=function(s){cleanup();try{c.abort(s.reason)}catch(e){c.abort()}};var cleanup=function(){for(var i=0;i<l.length;i++){try{l[i].removeEventListener("abort",on)}catch(e){}}};for(var i=0;i<l.length;i++){try{l[i].addEventListener("abort",on,{once:true})}catch(e){}}}return c.signal}}if(typeof S.timeout!=="function"){S.timeout=function(ms){var c=new self.AbortController();setTimeout(function(){try{c.abort(new Error("The operation timed out."))}catch(e){c.abort()}},Math.max(0,Number(ms)||0));return c.signal}}}}catch(e){}}();</script>`;
