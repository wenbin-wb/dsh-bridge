...
class BridgeService {
  ...
  async saveCloudflaredConfig({ token, hostname }) {
    ...
    this.cloudflaredConfig = {
      ...prev,
      // '******' 是非管理员视图的掩码回显：原样传回时保留真实值，防止真实 Token 被覆盖
      token: token === '******' ? (prev.token ?? '') : (token ? String(token).trim() : ''),
  ...
  const disposeRpc = installBridgeRpc(ctx, {
    ...
    saveCustomTunnelConfig: async (serverUrl, accessToken) => {
      ...
      const next = {
        ...
        // '******' 是非管理员视图的掩码回显：原样传回时保留真实值，防止覆盖
        accessToken: accessToken === '******' ? (prev.accessToken ?? '') : accessToken,
      };
      stored.customTunnel = next;
      await saveConfig(stored);
      service.customTunnelConfig = next;
    },
    exportBackup: async () => {
      const stored = await loadConfig();
      return {
