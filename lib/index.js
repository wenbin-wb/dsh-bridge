...
class BridgeService {
  ...
  async checkVersion() {
    return new Promise((resolve) => {
      const { get } = require('node:https');  // eslint-disable-line
      const url = 'https://registry.npmjs.org/dsh-bridge/latest';
      const req = get(url, { timeout: 8000 }, (res) => {
        const chunks = [];
...
function apply(ctx, config = {}) {
  ...
  if (!dshPort) {
    logger.error('dsh-bridge: webServer port unavailable — cannot start proxy');
    return;
  }
