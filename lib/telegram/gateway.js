...
function renderAsciiTable(rows) {
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    for (let c = 0; c < numCols; c++) {
      const cell = String(row[c] ?? '').trim()
      formattedCells.push(' ' + padEndDisplay(cell, colWidths[c]) + ' ')
    }
...
export function formatTelegramHtml(text) {
  ...
  // 4. 提取并转换 Markdown 表格
  const tables = []
  const tableLines = safeText.split(/\r?\n/)
  ...
  const flushTable = () => {
    ...
    for (const tl of tableBuffer) {
      if (isDelimiterLine(tl)) continue
      const raw = tl.trim().slice(1, -1)
      const cells = raw.split('|').map((c) => c.trim())
      parsedRows.push(cells)
    }
    if (parsedRows.length > 0) {
      const idx = tables.length
      tables.push(renderAsciiTable(parsedRows))
      processedLines.push(`%%TG_TABLE_${idx}%%`)
    }
    tableBuffer = []
  }
  ...
  // 6. 转换 Markdown 标题 (#, ##, ###, ####, etc.) 为加粗标题
  safeText = safeText.replace(/^#{1,6}\s+(.+)$/gm, '<b>$1</b>')

  // 7. 转换 Markdown 引用块 (> text 或 &gt; text) 为 <blockquote>
  safeText = safeText.replace(/^(?:&gt;|>)\s*(.+)$/gm, '<blockquote>$1</blockquote>')

  // 8. 转换无序列表符号 (* item, - item) 为友好圆点 •
  ...
  // 11. 还原表格为等宽代码块
  safeText = safeText.replace(/%%TG_TABLE_(\d+)%%/g, (_, idx) => {
    const asciiTable = tables[Number(idx)] || ''
    const escaped = asciiTable.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
...
export class TelegramGateway extends Service {
  ...
  async registerCommands() {
    ...
    try {
      ...
      this.logger.info?.('[dsh-bridge telegram] bot command menu registered successfully')
    } catch (err) {
