...
export function formatTelegramHtml(text) {
  ...
  // 提取并保护代码块
  const codeBlocks = []
  let safeText = text.replace(/```([\s\S]*?)```/g, (_, code) => {
  ...
  // 提取并保护行内代码
  const inlineCodes = []
  safeText = safeText.replace(/`([^`]+)`/g, (_, code) => {
  ...
  // 还原行内代码（HTML 转义其内容）
  safeText = safeText.replace(/___INLINE_CODE_(\d+)___/g, (_, idx) => {
  ...
  // 还原代码块
  safeText = safeText.replace(/___CODE_BLOCK_(\d+)___/g, (_, idx) => {
