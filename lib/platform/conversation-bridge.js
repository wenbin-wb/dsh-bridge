  ...
  switch (command) {
    ...
    case 'workspaces': {
      ...
      if (workspaces.length === 0) {
        await node.sendText(`### 🗂️ 可用工作区\n\n> ℹ️ 当前没有已注册的工作区。可使用 \`/new <提示词> @<路径>\` 指定项目目录。`)
        return true
      }
...
async function renderSessions(node) {
  ...
  for (const [cwd, sessions] of sortedGroups) {
    ...
    if (sessions.length > 20) {
      parts.push(`  *…该工作区共 ${sessions.length} 个会话*`)
    }
    parts.push('')
  }
...
function helpText() {
  return [
    ...
    '- `/new <提示词> @序号` — 在指定工作区创建新会话（/workspaces）',
    '- `/new <提示词> @路径` — 在指定目录路径创建新会话',
