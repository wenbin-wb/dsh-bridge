...
export class ConversationBridge {
   ...
   * @param {object} [opts.platform]     所属 Platform 实例（提供 accountId/capabilities）
    ...
    try {
      ...
      if (cwdOverride) {
        ...
        if (!allowedPaths.includes(resolvedCwd)) {
          ...
          await this.sendText(
            `${this.mark.err} 路径不在已注册工作区内: ${resolvedCwd}\n\n` +
            `可用工作区：\n  ${wsDisplay}${more}\n\n` +
        ...
        if (!ok) {
          await this.sendText(`${this.mark.err} 工作区目录不存在: ${resolvedCwd}`)
          return
        }
      }
      const meta = {
  ...
  async handleInbound({ senderId, text, isGroup = false }) {
    ...
    if (!this.isAllowed(sender)) {
      ...
      if (this.config.allowFrom.length === 0 && !isGroup) {
        if (text?.trim()) {
          ...
          this.logger?.info?.(`[dsh-bridge ${this.platform?.id ?? 'im'}] auto-approved first sender ${sender} into allowlist (onboarding)`)
          try {
            await this.onFirstSender?.(sender)
          } catch (err) {
            this.logger?.warn?.(`[dsh-bridge ${this.platform?.id ?? 'im'}] failed to persist first sender: ${err instanceof Error ? err.message : String(err)}`)
          }
        } else {
          this.logger?.info?.(`[dsh-bridge ${this.platform?.id ?? 'im'}] media-only first message from ${sender} not auto-approved (waiting for text)`)
        }
      }

      // 如果仍未通过白名单，拒绝处理（防止绕过白名单）
      if (!this.isAllowed(sender)) {
        this.logger?.info?.(`[dsh-bridge ${this.platform?.id ?? 'im'}] ignore message from non-allowlisted sender ${sender} (never fed to model)`)
        return 'ignored'
      }
    }

    if (isGroup) {
      this.logger?.info?.(`[dsh-bridge ${this.platform?.id ?? 'im'}] ignore group message from ${sender} (v0.1: no group support)`)
      return 'ignored'
    }

    const fullText = text?.trim() ?? ''
    if (!fullText) {
      this.logger?.info?.(`[dsh-bridge ${this.platform?.id ?? 'im'}] ignore empty message from ${sender}`)
      return 'ignored'
    }
    ...
    if (!agent && this.activeSessionId) {
      ...
      try {
        ...
        agent = handle.agent
        this.logger?.info?.(`[dsh-bridge ${this.platform?.id ?? 'im'}] re-attached agent to session ${this.activeSessionId} (${persisted ? 'resume' : 'create'})`)
      } catch (err) {
        const reason = err instanceof Error ? err.message : String(err)
        this.logger?.warn?.(`[dsh-bridge ${this.platform?.id ?? 'im'}] failed to re-attach agent: ${reason}`)
    ...
    try {
      for (let i = 0; i < chunks.length; i++) {
        ...
        if (result && result.success === false) {
          this.logger?.warn?.(`[dsh-bridge ${this.platform?.id ?? 'im'}] outbound chunk ${i + 1}/${chunks.length} failed: ${result.error}`)
          break
        }
      ...
      if (atMatch) {
        ...
        if (/^\d+$/.test(sel)) {
          ...
          if (!allowedPaths.includes(normalized)) {
            ...
            const more = allowedPaths.length > 5 ? `\n  ... 等 ${allowedPaths.length} 个工作区` : ''
            await node.sendText(
              `${node.mark.err} 路径不在已注册工作区内: ${normalized}\n\n` +
              `可用工作区：\n  ${wsDisplay}${more}\n\n` +
              `提示：使用 /workspaces 查看完整列表`
            )
            return true
