...
export class ConversationBridge {
  ...
  // 因此必须以 { prepend: true } 注册到链条最外层：先发 IM 卡片，再调 next()
  // 让 GUI 弹窗并行挂起，两端谁先决议谁生效。

  _attachApprovalBridge() {
    const listener = async (req, next) => {
      const owned = this.ownsAgent(req.agent)
      if (!owned) {
        this.logger?.debug?.('[dsh-bridge %s] approval/request ignored: not our agent (session=%s)', this.platform?.id, req?.agent?.session?.id ?? '(none)')
        return next?.()
      }
      // 审批绑定到发起本轮的会话 peer；决议校验（_approvalAllowed）使用轮次发起成员
      const turn = req.agent?.session?.id ? this._turnPeers.get(req.agent.session.id) : null
      const peer = turn?.outboundPeer?.peerId ?? this.peerId
      const initiator = turn?.senderId ?? this.peerId
      if (!peer) {
      ...
      // Web GUI 通道：调用 next() 让链条后续（宿主 GUI 弹窗）并行挂起等待网页回答
      ...
      try {
        ...
        // GUI 通道异常（如 web 断连导致 next() 链 reject）不应中断审批：回退等待 IM 决议
        this.logger?.warn?.('[dsh-bridge %s] approval #%d web channel failed (%s), falling back to IM', this.platform?.id, number, err?.message ?? err)
      ...
      this.logger?.info?.('[dsh-bridge %s] approval #%d resolved: outcome=%s winner=%s', this.platform?.id, number, outcome, winner)
