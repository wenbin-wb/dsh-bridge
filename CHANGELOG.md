...
- **🚦 并发模型修复**：
  ...
  - 配置持久化读-改-写整事务化，多平台并发保存不再丢失更新；  - **修复 IM 端收不到审批卡片的问题**：DSH 宿主的审批分发是顺序瀑布（web GUI 认领监听器先行否决），桥接审批桥此前永远没有机会执行，导致微信/QQ 等 IM 端工具审批全部以"用户未批准"失败；现以 prepend 方式注册到分发链最外层——先向 IM 发送审批卡片、再让 GUI 弹窗并行挂起，两端谁先决议谁生效，并支持任务停止时同步取消待决审批；
