...
> **本次为远程锁屏/解锁流程修复版**：重点修复远程访问场景下锁屏不显示、解锁后无限循环、会话失效后无法恢复等一连串问题，并新增「管理保护」独立开关与 DSH 宿主版本显示。建议升级前备份 `~/.dsh/dsh-bridge/config.json`。

### 🔐 远程锁屏/解锁流程修复

- **修复解锁后无限锁屏循环**：DSH 重启或访问会话过期后，旧 cookie 失效，所有接口被代理以 HTTP 401 拦截。此前即使管理密码解锁成功（`authAdminUnlock` 已豁免），解锁后 `getStatus` 依旧 401 → 面板把锁重置回 true → 无限循环。现在检测到访问会话失效（HTTP 401 / transport failure）时，**整页重载回绿色图标登录页**重新输入访问密码获取新会话，不再困在锁屏；带防循环保护（重载一次未恢复则提示手动刷新）。
- **修复锁屏状态下无法解锁（401）**：锁屏时访问会话可能已失效，`authAdminUnlock` 请求此前也被 HTTP 访问认证拦截，导致锁屏永远解不开；现豁免该端点的访问认证（内部仍校验管理密码，安全不受影响）。
- **修复关闭「访问认证」后锁屏不显示**：锁屏此前依赖访问认证开关，关闭后远程设备不再锁定（管理保护形同虚设）；现管理保护独立生效，关闭访问认证后远程设备仍显示锁屏，仅凭管理密码可解锁。
- **修复关闭「访问认证」后仍要求密码**：`enabled` 开关豁免管理 gate（否则"关闭访问认证"这个动作本身会被管理保护锁死）。
- **修复远程首次进入设置页不显示锁屏**：现在远程设备首次进入即显示完整全屏锁屏（输入管理密码后方可操作）。
- **修复远程目录选择器卡死**：目录浏览被鉴权拦截时内嵌解锁入口，解锁后自动继续；`local_only` 策略下明确提示远程不可解锁。
- **修复解锁成功后操作丢失**：被拦截的操作暂存队列，解锁成功后自动重放，无需重新点击。

### ✨ 新功能

- **新增「管理保护」独立开关**：与访问认证解耦的独立安全开关（安全认证页可切换）。关闭后管理操作免 adminToken，与访问认证开关互不干扰。
