...
### 🔐 访问安全认证系统 + 全局后台防篡改锁 + 三重容灾保命体系 + Tab 面板全面升级

**重磅引入企业级全协议访问安全防护、管理后台防篡改控制、物理特权与救急指令三重保命机制，并深度重构 Tab 面板与深色模式体验**

#### ✨ 新特性 (Features)

- **🛡️ 第一道防线：多通道访问门禁认证 (Access Auth Guard)**：
...
- **🛟 三重容灾保命体系 (Fail-safe Recovery System)**：
...
- **🎨 界面与体验深度重构**：
  - **4 大独立 Tab 布局**：`局域网` -> `公网隧道` -> `IM 机器人` -> `🔐 安全认证`，层级分明；
...
### 🚀 Telegram 机器人官方对接 + 打字机流式输出 + 原生指令菜单与交互卡片

**全新接入 Telegram 平台，支持零公网长轮询、HTTP/HTTPS 原生代理隧道、轮次增量打字机流式、原生快捷菜单与交互审批卡片**

#### ✨ 新特性 (Features)

- **Telegram 平台全生命周期支持**：
  - 基于官方 Long Polling（长轮询 `getUpdates`）实现 100% 免公网 IP、免 Webhook 接入；
  - 内置零外部依赖的 HTTP/HTTPS CONNECT 隧道代理支持，国内网络环境下填入代理地址（如 `http://127.0.0.1:7890`）即可极速通信；
  - 自动管理 Bot Token 凭证、会话恢复、长连接自适应重试与状态指示。
- **实时增量打字机流式输出**：
  - 接入 Agent 生成轮次实时生命周期（`turn/start` / `turn/end`）；
  - 基于 `splitIntoIncremental` 与 Telegram `editMessageText` 实现单条气泡原地打字机更新，告别频繁发送碎气泡。
- **原生快捷指令菜单（Menu 按钮）**：
  - 自动通过 `setMyCommands` 与 `setChatMenuButton` 注册全范围指令（`/new`, `/sessions`, `/use`, `/workspaces`, `/status`, `/stop`, `/end`, `/help`）；
  - 输入 `/` 或点击左下角 `[Menu]` 按钮一键直达常用命令。
- **Inline Keyboard 交互卡片与审批按键**：
  - 敏感操作审批下发 `[✓ 批准执行]` / `[✕ 拒绝执行]` 按键，一键点击即时响应；
  - 帮助与状态消息附带一键交互按键（查看会话、工作区、停止任务等），支持 Callback Query 极速响应。
- **多模态图片与文件收发**：
  - 入站照片与文档自动下载转存并传递给 Agent；
  - 出站工具产物文件与图片在轮次结束时自动推回 Telegram 聊天框。
- **优雅 Markdown 解析排版**：
  - 针对 Telegram HTML 模式深度定制，支持加粗标题、列表实心圆点、引用块、超链接与等宽代码高亮。

#### 🐛 修复与优化 (Fixes)

- **白名单持久化**：修复 Telegram 会话节点首次发送消息自动加入白名单未持久化写入配置的问题；
- **Web 控制台适配**：修复 Telegram 表单校验在无 appId 场景下按钮被禁用的问题；
- **全套测试覆盖**：新增 Telegram 网关、服务、命令菜单、卡片交互与流式输出 96 个单元测试全部通过。

---

## 2.3.0

### 🚀 飞书 (Feishu / Lark) 官方长连接机器人接入 + Card 2.0 交互卡片审批
