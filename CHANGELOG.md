...
- **全回复流式 Markdown**：
  - 所有 AI 回复统一走流式接口（不再区分长短走普通消息）
  - 流式内容用 `content_type: markdown`，手机端正常渲染 Markdown（粗体/代码块/列表等）
  - 短消息自动拆两片保证「生成中 → 生成结束」流式过渡
  - 分段按段落边界切分，不切断行内内容
- **QQ 安全 Markdown 转换**：
...
- 单元测试从 56 → 64（新增域名、流式路径、面板/菜单端点、键盘消息、输入状态、Markdown 转换、分片 7 组）

#### 文档

- 更新 `docs/qq-usage.md`：新增指令面板与自定义菜单说明、输入状态实现方式、流式 Markdown 正确用法
